...
// Read context (matching discounts.php pattern)
...
<div id="smPageContainer" class="page-container full-page-admin" dir="<?= $dir ?>">
    ...
    <div class="card-body" style="padding: clamp(8px, 1.5vw, 12px) clamp(12px, 2vw, 16px);">
      <div class="filters-grid">
        <div class="filter-group filter-group--search">
          <label class="filter-label" for="searchInput"><?= _smt('filter.search', 'Search') ?></label>
          <input type="text" class="form-control" id="searchInput"
                 placeholder="<?= _smt('filter.search_placeholder', 'Search by product name or SKU...') ?>">
        </div>
        <div class="filter-group">
          <label class="filter-label" for="typeFilter"><?= _smt('filter.type', 'Type') ?></label>
        ...
        <div class="filter-group">
          <label class="filter-label" for="dateFrom"><?= _smt('filter.date_from', 'From Date') ?></label>
          <input type="date" class="form-control" id="dateFrom" title="<?= _smt('filter.date_from', 'From Date') ?>">
        </div>
        <div class="filter-group">
          <label class="filter-label" for="dateTo"><?= _smt('filter.date_to', 'To Date') ?></label>
          <input type="date" class="form-control" id="dateTo" title="<?= _smt('filter.date_to', 'To Date') ?>">
        </div>
        <div class="filter-group">
          <label class="filter-label" aria-hidden="true">&nbsp;</label>
          <div class="filter-buttons">
            <button class="btn btn-sm btn-primary" id="btnFilter" data-btn-slug="primary"
                    title="<?= _smt('filter.apply', 'Filter') ?>" aria-label="<?= _smt('filter.apply', 'Filter') ?>">
              <i class="fas fa-search" aria-hidden="true"></i>
            </button>
            <button class="btn btn-sm btn-secondary" id="btnClearFilter" data-btn-slug="secondary"
                    title="<?= _smt('filter.clear', 'Clear Filters') ?>" aria-label="<?= _smt('filter.clear', 'Clear Filters') ?>">
  ...
  <!-- Add / Edit Movement Modal (inside container for CSS token scope) -->
  <div class="sm-modal-backdrop" id="movementModal" style="display:none" role="dialog" aria-modal="true" aria-labelledby="modalTitle">
    <div class="sm-modal-panel">
      <div class="sm-modal-header">
        <h3 id="modalTitle"><?= _smt('add_movement', 'Add Movement') ?></h3>
        <button class="sm-modal-close" id="btnCloseModal" aria-label="Close">&times;</button>
      </div>
        ...
        <form id="movementForm">
          ...
          <div class="form-group">
            ...
            <div class="input-group">
              ...
              <button type="button" class="btn btn-sm btn-secondary" id="btnLookupProduct" data-btn-slug="secondary">
                <i class="fas fa-search" aria-hidden="true"></i>
              </button>
            </div>
      ...
      <div class="sm-modal-footer">
        <button type="button" class="btn btn-secondary" id="btnCancelModal" data-btn-slug="secondary">
          <?= _smt('form.cancel', 'Cancel') ?>
        </button>
        <button type="button" class="btn btn-primary" id="btnSaveMovement" data-btn-slug="primary">
          <?= _smt('form.save', 'Save') ?>
        </button>
      </div>
    </div>
