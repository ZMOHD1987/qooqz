...
(function () {
  ...
  // Debug CSRF
  if (DEBUG) {
    console.log('[vendors.js] CSRF Token available:', CSRF_TOKEN ? 'Yes' : 'No');
    console.log('[vendors.js] CSRF Token value:', CSRF_TOKEN);
    console.log('[vendors.js] Is Admin:', IS_ADMIN);
  }
  ...
  function getCsrfToken() {
    ...
    if (csrfInput && csrfInput.value) {
      ...
      log('Got CSRF from hidden input:', CSRF_TOKEN.substring(0, 20) + '...');
      return CSRF_TOKEN;
    }
    ...
    if (window.CSRF_TOKEN) {
      ...
      log('Got CSRF from window object:', CSRF_TOKEN.substring(0, 20) + '...');
      return CSRF_TOKEN;
    }
    
    // Try from ADMIN_UI
    if (window.ADMIN_UI && window.ADMIN_UI.csrf_token) {
      CSRF_TOKEN = window.ADMIN_UI.csrf_token;
      log('Got CSRF from ADMIN_UI:', CSRF_TOKEN.substring(0, 20) + '...');
      return CSRF_TOKEN;
    }
  ...
  async function postFormData(fd) {
    ...
    // Add CSRF token using set() - THIS IS CRITICAL
    if (csrfToken) {
      fd.set('csrf_token', csrfToken);
      log('CSRF token added to FormData:', csrfToken.substring(0, 20) + '...');
    } else {
      log('WARNING: No CSRF token available!');
    }
    
    // Debug FormData
    if (DEBUG) {
      console.log('Full CSRF Token:', csrfToken);
      const formDataObj = {};
      for (const [key, value] of fd.entries()) {
        formDataObj[key] = value instanceof File ? `[File: ${value.name}]` : value;
      }
      console.log('FormData to send:', formDataObj);
    }
  ...
  function setError(msg) {
    ...
    refs.errorsBox.style.padding = '12px';
    refs.errorsBox.style.backgroundColor = '#fee';
    refs.errorsBox.style.borderRadius = '6px';
    refs.errorsBox.style.color = '#b91c1c';
  }
  ...
  function collectFormData() {
    ...
    if (csrfToken) {
      fd.set('csrf_token', csrfToken);
      log('CSRF token in collectFormData:', csrfToken.substring(0, 20) + '...');
    }
    ...
    if (IS_ADMIN) {
      if ($('vendor_status')) fd.append('status', $('vendor_status').value || 'pending');
      if ($('vendor_is_verified')) fd.append('is_verified', $('vendor_is_verified').checked ? '1' : '0');
      if ($('vendor_is_featured')) fd.append('is_featured', $('vendor_is_featured').checked ? '1' : '0');
    }
    ...
    if (DEBUG) {
      ...
      console.log('FormData collected:', debugData);
    }
  ...
  async function saveVendor() {
    ...
    const csrfToken = getCsrfToken();
    if (!csrfToken) {
      setError(t('csrf_missing', 'Security token missing. Please refresh the page.'));
      showNotification(t('csrf_error', 'Security error. Please refresh page.'), 'error');
      return;
    }
    ...
    try {
      ...
      if (error.status === 403) {
        if (error.body && error.body.message && error.body.message.includes('CSRF')) {
          setError(t('csrf_invalid', 'Security token expired. Please refresh the page.'));
          showNotification(t('session_expired', 'Session expired. Refreshing...'), 'error');
          setTimeout(() => location.reload(), 2000);
        } else {
          showNotification(t('forbidden', 'Forbidden - no permission'), 'error');
        }
        return;
      }
  ...
  (async function init() {
    ...
    // Get CSRF token early
    const csrfToken = getCsrfToken();
    if (!csrfToken) {
      console.error('CRITICAL: No CSRF token found. Page may not work correctly.');
      showNotification(t('csrf_missing_init', 'Security token missing. Page may not work.'), 'warning', 10000);
    } else {
      log('CSRF token initialized:', csrfToken.substring(0, 20) + '...');
    }
    ...
    // Test CSRF function (for debugging)
    if (DEBUG) {
      window.testCsrf = async function() {
        ...
        try {
          ...
          showNotification('CSRF Test: ' + (data.success ? 'PASS' : 'FAIL'), data.success ? 'success' : 'error');
        } catch (err) {
          console.error('CSRF Test Error:', err);
          showNotification('CSRF Test Error', 'error');
        }
      };
      ...
      testBtn.style.cssText = 'position:fixed;bottom:10px;right:10px;z-index:9999;padding:5px;background:#666;color:white;border:none;border-radius:4px;font-size:12px;';
      testBtn.onclick = window.testCsrf;
