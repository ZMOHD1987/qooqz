...
// Complete, improved admin product script with full translation support,
// cascade categories, attributes, variants, media studio integration.
// Requires globals from PHP fragment:
// - window.CSRF_TOKEN
// - window.CURRENT_USER
// - window.AVAILABLE_LANGUAGES
//
// Place at: admin/assets/js/pages/products.js
...
(function () {
  ...
  const AVAILABLE_LANGUAGES = window.AVAILABLE_LANGUAGES || [{ code: 'en', name: 'English' }];
  ...
  function getPreferredLang() {
    try {
      ...
      return (u.preferred_language && String(u.preferred_language).trim()) || PREF_LANG || 'en';
    } catch (e) {
      return PREF_LANG || 'en';
    }
  }
  ...
  function getTranslatedName(item) {
    ...
    if (item.translations && typeof item.translations === 'object') {
      ...
      if (t && (t.name || t.title)) return t.name || t.title;
    }
    if (item.name_translated) return item.name_translated;
    if (item.title_translated) return item.title_translated;
    return item.name || item.title || item.sku || '';
  }

  function getTranslatedField(item, field) {
    const lang = getPreferredLang();
    if (!item) return '';
    if (item.translations && item.translations[lang] && item.translations[lang][field]) return item.translations[lang][field];
    if (item[field]) return item[field];
    if (item[field + '_translated']) return item[field + '_translated'];
    return '';
  }

  // ---------- Meta (categories, attributes, brands) ----------
  function loadMeta(lang, categoryFilter = []) {
    lang = lang || getPreferredLang();
    let url = META_API + '?lang=' + encodeURIComponent(lang);
    if (Array.isArray(categoryFilter) && categoryFilter.length) url += '&categories=' + encodeURIComponent(categoryFilter.join(','));
  ...
  function populateMeta() {
    ...
    if (attrSelect) {
      ...
      (META.attributes || []).forEach(a => {
        ...
        o.dataset.isVariation = a.is_variation ? '1' : '0';
        attrSelect.appendChild(o);
      });
    }

    // categories tree
    buildCategoryTreeUI(META.categories || []);
  }

  // Build category tree into #categoryList
  function buildCategoryTreeUI(nodes) {
    if (!categoryList) return;
    categoryList.innerHTML = '';
    function walk(list, level = 0, parentUl = categoryList) {
      list.forEach(node => {
        const li = document.createElement('li');
        li.style.marginLeft = (level * 12) + 'px';
        li.style.marginTop = '6px';
        const id = node.id;
        const name = node.name_translated || node.name || node.slug || id;
        ...
        if (node.children && node.children.length) {
          ...
          childUl.style.marginTop = '6px';
          childUl.style.display = 'none';
          toggle.addEventListener('click', () => {
            if (childUl.style.display === 'none') { childUl.style.display = 'block'; toggle.textContent = '-'; }
            else { childUl.style.display = 'none'; toggle.textContent = '+'; }
          });
          li.appendChild(toggle);
          li.appendChild(childUl);
          parentUl.appendChild(li);
          walk(node.children, level + 1, childUl);
        } else {
          parentUl.appendChild(li);
        }
      });
  ...
  function addAttributeItem(attrId, valId, custom) {
    ...
    Array.from(attrSelect.options).forEach(o => { if (!o.value) return; sel.appendChild(o.cloneNode(true)); });
  ...
  function collectCategories() {
    const arr = [];
    const primary = categoryList.querySelector('input[type="radio"]:checked');
    if (primary) {
      arr.push(parseInt(primary.value, 10));
    ...
    Array.from(checks).forEach(ch => {
      if (primary && ch.value === primary.value) return;
      arr.push(parseInt(ch.value, 10));
    });
  ...
  function addVariantRow(v = {}) {
    ...
    const sku = document.createElement('input'); sku.type = 'text'; sku.placeholder = 'SKU'; sku.value = v.sku || ''; sku.style.width = '120px';
    const qty = document.createElement('input'); qty.type = 'number'; qty.placeholder = 'Stock'; qty.value = v.stock_quantity || 0; qty.style.width = '80px';
    const price = document.createElement('input'); price.type = 'text'; price.placeholder = 'Price'; price.value = v.price || ''; price.style.width = '80px';
    const activeLabel = document.createElement('label'); activeLabel.textContent = 'Active ';
    const active = document.createElement('input'); active.type = 'checkbox'; active.checked = v.is_active !== 0;
    activeLabel.appendChild(active);
    const remove = document.createElement('button'); remove.type = 'button'; remove.className = 'btn small danger'; remove.textContent = 'Remove';
    remove.addEventListener('click', () => row.remove());

    row.appendChild(sku); row.appendChild(qty); row.appendChild(price); row.appendChild(activeLabel); row.appendChild(remove);
  ...
  function generateVariants() {
    ...
    const attrValues = attrs.map(a => {
      const attr = META.attributes.find(at => String(at.id) === String(a.attribute_id));
      return (attr.values || []).map(v => ({ attrId: a.attribute_id, valId: v.id, label: v.value }));
    });

    function cartesian(arrays) {
      return arrays.reduce((a, b) => a.flatMap(d => b.map(e => [ ...(Array.isArray(d) ? d : [d]), e ].flat())));
    }

    const combos = cartesian(attrValues);
    variantsList.innerHTML = '';
    combos.forEach(() => addVariantRow({ sku: '', stock_quantity: 0, price: '', is_active: 1 }));
  }
  ...
  // ---------- Translations ----------
  function addTranslationPanel(langCode, displayName) {
    // If displayName not provided, request via prompt (only when user clicks Add Language)
    let langName = displayName;
    if (!langName) {
      langName = prompt('Language name (display, e.g. Arabic)', langCode || '');
      if (!langName) return;
    }
    if (!langCode) langCode = prompt('Language code (e.g. ar)', '') || '';
    if (!langCode) return;
    // Avoid duplicates
    if (document.querySelector(`.tr-lang-panel[data-lang="${langCode}"]`)) return;
  ...
  function collectTranslations() {
    ...
    translationsArea.querySelectorAll('.tr-lang-panel').forEach(panel => {
      ...
      const name = (panel.querySelector('.tr-name') || { value: '' }).value.trim();
      const shortt = (panel.querySelector('.tr-short') || { value: '' }).value.trim();
      const desc = (panel.querySelector('.tr-desc') || { value: '' }).value.trim();
      const spec = (panel.querySelector('.tr-spec') || { value: '' }).value.trim();
      const mt = (panel.querySelector('.tr-meta-title') || { value: '' }).value.trim();
      const md = (panel.querySelector('.tr-meta-desc') || { value: '' }).value.trim();
      const mk = (panel.querySelector('.tr-meta-keys') || { value: '' }).value.trim();
  ...
  function loadMedia() {
    ...
    fetchJson(MEDIA_API + '?user_id=' + encodeURIComponent(USER.id)).then(res => {
      if (res.success) MEDIA = res.data || [];
    }).catch(err => { console.error('media load error', err); });
  }

  function openMediaStudio() {
    // Simple modal to pick images from MEDIA
    const modal = document.createElement('div');
    modal.style.position = 'fixed'; modal.style.left = 0; modal.style.top = 0;
    modal.style.right = 0; modal.style.bottom = 0; modal.style.background = 'rgba(0,0,0,0.5)';
    modal.style.display = 'flex'; modal.style.alignItems = 'center'; modal.style.justifyContent = 'center'; modal.style.zIndex = 9999;

    const panel = document.createElement('div');
    panel.style.width = '80%'; panel.style.maxWidth = '900px'; panel.style.maxHeight = '80%'; panel.style.overflow = 'auto';
    panel.style.background = '#fff'; panel.style.padding = '16px'; panel.style.borderRadius = '8px';

    const title = document.createElement('h3'); title.textContent = 'Media Studio';
    panel.appendChild(title);

    const grid = document.createElement('div'); grid.style.display = 'flex'; grid.style.flexWrap = 'wrap'; grid.style.gap = '8px';
    (MEDIA || []).forEach(m => {
      const d = document.createElement('div'); d.style.width = '120px'; d.style.height = '90px'; d.style.border = '1px solid #e6eef0'; d.style.overflow = 'hidden'; d.style.cursor = 'pointer';
      const im = document.createElement('img'); im.src = m.url; im.style.maxWidth = '100%'; im.style.maxHeight = '100%'; d.appendChild(im);
      d.addEventListener('click', () => {
        // append preview and hidden input
        const pv = document.createElement('div'); pv.style.width = '90px'; pv.style.height = '90px'; pv.style.border = '1px solid #e6eef0'; pv.style.borderRadius = '8px'; pv.style.overflow = 'hidden'; pv.style.display = 'flex'; pv.style.alignItems = 'center'; pv.style.justifyContent = 'center';
        const img = document.createElement('img'); img.src = m.url; img.style.maxWidth = '100%'; img.style.maxHeight = '100%'; pv.appendChild(img);
        imagesPreview.appendChild(pv);
        const hi = document.createElement('input'); hi.type = 'hidden'; hi.name = 'existing_media[]'; hi.value = m.id;
  ...
  function loadList() {
    ...
    fetchJson(API + '?format=json' + q).then(res => {
      ...
      renderTable(list);
      setNotice('');
    }).catch(err => { console.error(err); setNotice('Load failed: ' + (err.message || ''), true); });
  }
  ...
  function renderTable(list) {
    ...
    list.forEach(p => {
      const titleText = getTranslatedName(p);
      const tr = document.createElement('tr');
      tr.innerHTML = `<td style="padding:8px;">${escapeHtml(p.id)}</td>
        <td style="padding:8px;">${escapeHtml(titleText)}</td>
  ...
  function doDelete(id) {
    ...
    const fd = new FormData(); fd.append('action', 'delete'); fd.append('id', id); if (CSRF) fd.append('csrf_token', CSRF);
    fetch(API, { method: 'POST', body: fd, credentials: 'include' }).then(r => r.json()).then(j => {
  ...
  function toggleActive(id, newState) {
    const fd = new FormData(); fd.append('action', 'toggle_active'); fd.append('id', id); fd.append('is_active', newState ? 1 : 0); if (CSRF) fd.append('csrf_token', CSRF);
    fetch(API, { method: 'POST', body: fd, credentials: 'include' }).then(r => r.json()).then(j => {
  ...
  function openNew() {
    ...
    translationsArea.querySelectorAll('.tr-lang-panel').forEach(p => p.remove());
    // create default translation panel for preferred lang
    addTranslationPanel(getPreferredLang(), (AVAILABLE_LANGUAGES.find(l=>l.code===getPreferredLang())||{name:getPreferredLang()}).name);
    showErrors(null);
  ...
  function openEdit(id) {
    ...
    fetchJson(API + '?_fetch_row=1&id=' + encodeURIComponent(id)).then(res => {
      ...
      setNotice('');
    }).catch(err => { console.error(err); setNotice('Load error: ' + (err.message || ''), true); });
  }
  ...
  function populateForm(p) {
    ...
    if (input_barcode) input_barcode.value = p.barcode || '';
    if (select_type) select_type.value = p.product_type || 'simple';
    if (select_brand) select_brand.value = p.brand_id || '';
    if (select_manufacturer) select_manufacturer.value = p.manufacturer_id || '';
    if (input_published_at) input_published_at.value = p.published_at ? p.published_at.slice(0,16) : '';
    if (input_description) input_description.value = getTranslatedField(p, 'description') || p.description || '';
    if (input_price) input_price.value = (p.pricing && p.pricing.price) ? p.pricing.price : '';
    if (input_compare) input_compare.value = (p.pricing && p.pricing.compare_at_price) ? p.pricing.compare_at_price : '';
    if (input_cost) input_cost.value = (p.pricing && p.pricing.cost_price) ? p.pricing.cost_price : '';
    if (input_stock) input_stock.value = p.stock_quantity || 0;
    if (input_low) input_low.value = p.low_stock_threshold || 5;
    if (select_stock_status) select_stock_status.value = p.stock_status || 'in_stock';
    if (select_manage_stock) select_manage_stock.value = p.manage_stock ? 1 : 0;
    if (select_allow_backorder) select_allow_backorder.value = p.allow_backorder ? 1 : 0;
    if (input_tax) input_tax.value = p.tax_rate ?? '';
    if (input_weight) input_weight.value = p.weight ?? '';
    if (input_length) input_length.value = p.length ?? '';
    if (input_width) input_width.value = p.width ?? '';
    if (input_height) input_height.value = p.height ?? '';
    ...
    if (p.categories && p.categories.length) {
      ...
      p.categories.forEach(c => {
        const check = categoryList.querySelector(`input[type="checkbox"][value="${c.category_id}"]`);
    ...
    // translations: create panels for languages available in product translations
    translationsArea.querySelectorAll('.tr-lang-panel').forEach(n => n.remove());
    if (p.translations) {
      Object.keys(p.translations).forEach(lang => {
        const avail = AVAILABLE_LANGUAGES.find(l => l.code === lang);
        const displayName = avail ? avail.name : lang;
        addTranslationPanel(lang, displayName);
      ...
      // ensure at least preferred lang panel available
      addTranslationPanel(getPreferredLang(), (AVAILABLE_LANGUAGES.find(l=>l.code===getPreferredLang())||{name:getPreferredLang()}).name);
    }
    ...
    if (p.media && p.media.length) {
      p.media.forEach(m => {
        ...
        if (!url) return;
        const d = document.createElement('div'); d.style.width='90px'; d.style.height='90px'; d.style.border='1px solid #e6eef0'; d.style.borderRadius='8px'; d.style.overflow='hidden'; d.style.display='flex'; d.style.alignItems='center'; d.style.justifyContent='center';
        const im = document.createElement('img'); im.src = url; im.style.maxWidth='100%'; im.style.maxHeight='100%'; d.appendChild(im); imagesPreview.appendChild(d);
      });
    }
  ...
  if (fillFromDefaultBtn) fillFromDefaultBtn.addEventListener('click', function () {
    const v = input_name.value || '';
    if (!v) return alert('Default name is empty');
    translationsArea.querySelectorAll('.tr-name').forEach(i => { if (!i.value) i.value = v; });
  });

  if (addLangBtn) addLangBtn.addEventListener('click', () => addTranslationPanel('', ''));

  // delegate remove/collapse inside translations area
  translationsArea && translationsArea.addEventListener('click', function (e) {
    if (e.target.classList.contains('toggle-lang')) {
      const lang = e.target.dataset.lang;
      const body = translationsArea.querySelector(`.tr-lang-panel[data-lang="${lang}"] .tr-lang-body`);
      if (!body) return;
      body.style.display = body.style.display === 'none' ? 'block' : 'none';
      e.target.textContent = body.style.display === 'none' ? 'Open' : 'Collapse';
    } else if (e.target.classList.contains('remove-lang')) {
      const lang = e.target.dataset.lang;
      const pnl = translationsArea.querySelector(`.tr-lang-panel[data-lang="${lang}"]`);
      if (pnl) pnl.remove();
    }
  });

  // when categories change, optionally reload meta filtered by selected categories
  categoryList && categoryList.addEventListener('change', function () {
    const selected = Array.from(categoryList.querySelectorAll('input[type="checkbox"]:checked')).map(i => i.value);
    // update attributes based on selected categories (optional)
    if (selected.length) loadMeta(getPreferredLang(), selected);
  });

  // ---------- Init ----------
  Promise.all([loadMeta(getPreferredLang()), loadMedia()]).then(() => loadList());
