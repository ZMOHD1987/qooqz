/*!
 * /admin/assets/js/pages/stock_movements.js  v3.0
 *
 * Patterns:
 * • sm-toast-* notifications with container
 * • All buttons use data-btn-slug + Admin.buttons.applyHoverEffects()
 * • CSRF token sent on all mutating requests
 * • Modal driven by sm-modal-backdrop pattern
 */
(function () {
'use strict';
...
function loadMovements(page) {
    ...
    tbody.innerHTML = '<tr><td colspan="9" class="text-center">' + t('table.loading', 'Loading…') + '</td></tr>';
                ...
                d.data.items.forEach(function (item) {
                    ...
                    tr.innerHTML =
                        ...
                        '<td class="actions-cell">' +
                            (CFG.canEdit   ? '<button class="btn btn-sm btn-primary btn-edit" data-id="' + item.id + '" data-btn-slug="primary"><i class="fas fa-edit" aria-hidden="true"></i> ' + t('form.edit', 'Edit') + '</button> ' : '') +
                            (CFG.canDelete ? '<button class="btn btn-sm btn-danger btn-delete" data-id="' + item.id + '" data-btn-slug="danger"><i class="fas fa-trash" aria-hidden="true"></i> ' + t('form.delete', 'Delete') + '</button>' : '') +
                        '</td>';
                    tbody.appendChild(tr);
                });
                renderPagination(d.data);
                // Apply DB-driven hover effects to dynamically created buttons
...
function deleteMovement(id) {
    ...
    fetch('/api/product_stock_movements?id=' + id, { method: 'DELETE', headers: authHeaders() })
        .then(function (r) { return r.json(); })
        .then(function (d) {
            if (d.success) {
