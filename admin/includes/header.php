...
<head>
    ...
    <!-- Admin Framework (includes button base styles) -->
    ...
    <!-- Dynamic Theme CSS (from DB, e.g., generated_css) -->
...
<?= $theme['generated_css'] ?>
    ...
    <!-- CSS Variables — DB-driven (both underscore and hyphen forms) -->
...
<?php endforeach; ?>
<?php
// ── Alias vars for framework compatibility ───────────────────
...
foreach ($themeAliasVars as $varName => $varValue):
    if (!$varValue || !empty($_emittedVars[$varName])) continue;
?>    <?= $varName ?>: <?= $varValue ?>;
<?php endforeach; ?>
}
...
.admin-sidebar {
    ...
    <?php
    // ════════════════════════════════════════════════════════════
    // GENERATE BUTTON STYLES FROM DATABASE
    // ════════════════════════════════════════════════════════════
    function generate_button_styles_css($db, $tenantId) {
    if (!$db) return '';

    // Map button slugs to their corresponding CSS variables
    $buttonColorMap = [
        'primary' => [
            'bg' => '--primary-color',
        ...
        'outline' => [
            'bg' => 'transparent',
            'hover_bg' => 'var(--primary-color)',
            'text' => 'var(--primary-color)',
            'hover_text' => 'var(--text-primary)',
            'border' => 'var(--primary-color)',
            'hover_border' => 'var(--primary-color)',
        ],
        'link' => [
            'bg' => 'transparent',
            'hover_bg' => 'transparent',
            'text' => 'var(--primary-color)',
            'hover_text' => 'var(--primary-hover)',
            'border' => 'transparent',
            'hover_border' => 'transparent',
        ],
        'ghost' => [
            'bg' => 'transparent',
            'hover_bg' => 'color-mix(in srgb, var(--primary-color) 10%, transparent)',
            'text' => 'var(--primary-color)',
            'hover_text' => 'var(--primary-hover)',
            'border' => 'var(--primary-color)',
            'hover_border' => 'var(--primary-color)',
    ...
    try {
        $stmt = $db->prepare("SELECT slug, border_radius, padding, font_size, font_weight,
                                     border_width
                              FROM button_styles
        ...
        $css = "/* Dynamically generated button styles from button_styles table */\n";
        foreach ($styles as $style) {
            $slug = htmlspecialchars($style['slug']);
            $borderRadius = (int)$style['border_radius'];
            $padding = htmlspecialchars($style['padding']);
            $fontSize = htmlspecialchars($style['font_size']);
            $fontWeight = htmlspecialchars($style['font_weight']);
            $borderWidth = (int)$style['border_width'];

            // Get color mapping for this slug, fallback to primary if unknown
            $colors = $buttonColorMap[$slug] ?? $buttonColorMap['primary'];

            // Build CSS for .btn-{$slug}
            $css .= ".btn-{$slug} {\n";
            $css .= "    background-color: {$colors['bg']};\n";
            $css .= "    color: {$colors['text']};\n";
            $css .= "    border: {$borderWidth}px solid {$colors['border']};\n";
            ...
            // Hover state
            $css .= ".btn-{$slug}:hover:not(:disabled) {\n";
            if (isset($colors['hover_bg'])) {
                $css .= "    background-color: {$colors['hover_bg']};\n";
            }
            if (isset($colors['hover_text'])) {
                $css .= "    color: {$colors['hover_text']};\n";
            } else {
                $css .= "    color: {$colors['text']};\n";
            }
            if (isset($colors['hover_border'])) {
                $css .= "    border-color: {$colors['hover_border']};\n";
            } else {
                $css .= "    border-color: {$colors['border']};\n";
            }
            $css .= "}\n";
        }
        ...
        error_log("[header.php] Failed to generate button styles: " . $e->getMessage());
        return '';
    }
}
    ...
    // Output button styles if possible
    if ($db && $tenantId) {
        echo generate_button_styles_css($db, $tenantId);
    } else {
        // Debug output (in comments) to help diagnose
        echo "<!-- Button styles not generated: db=" . ($db ? 'ok' : 'missing') . ", tenantId=" . ($tenantId ?: 'missing') . " -->\n";
    }
    ?>
