...
// Expect connectDB() to be defined in api/config/db.php and return mysqli
...
// Priority: translations table (namespace approach) -> languages file (/admin/languages/admin/{lang}.json or /languages/admin/{lang}.json) -> existing $ui_strings
if (isset($mysqli) && ($mysqli instanceof mysqli)) {
    try {
        // try to load translations from translations table if exists
        ...
        if ($res && $res->num_rows > 0) {
            ...
            if ($stmt) {
                ...
                while ($row = $r->fetch_assoc()) {
                    ...
                    $key = $row['t_key'];
                    $val = $row['value'];
                    if (!isset($ui_strings[$ns]) || !is_array($ui_strings[$ns])) $ui_strings[$ns] = [];
                    $ui_strings[$ns][$key] = $val;
                }
                $stmt->close();
            }
        } else {
            // fallback to language file below if table not present
        }
    } catch (Throwable $e) {
...
// If translations still sparse, attempt to load language file from common paths
$possibleLangPaths = [
    rtrim($_SERVER['DOCUMENT_ROOT'] ?? '', '/\\') . '/admin/languages/admin/' . preg_replace('/[^a-z0-9_-]/i','', $preferred_lang) . '.json',
    rtrim($_SERVER['DOCUMENT_ROOT'] ?? '', '/\\') . '/languages/admin/' . preg_replace('/[^a-z0-9_-]/i','', $preferred_lang) . '.json',
    __DIR__ . '/../languages/admin/' . preg_replace('/[^a-z0-9_-]/i','', $preferred_lang) . '.json',
];
...
foreach ($possibleLangPaths as $langFile) {
    if (is_readable($langFile)) {
        ...
        if (is_array($decoded)) {
            // merge decoded into ui_strings (decoded overrides existing keys)
...
// ui_t: resolve translation key like "users.headers.username" or "strings.title"
...
if (!function_exists('enqueue_css')) {
    function enqueue_css(string $href): void {
        ...
        // if headers already sent, output immediate fallback
        if (headers_sent()) {
...
if (!function_exists('print_admin_ui_js')) {
    function print_admin_ui_js(): void {
        global $ui_strings;
        // sanitize and output
        $json = json_encode($ui_strings, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
        if ($json === false) $json = '{}';
        echo '<script>window.ADMIN_UI = window.ADMIN_UI || {}; Object.assign(window.ADMIN_UI, ' . $json . '); window.ADMIN_LANG = window.ADMIN_UI.lang || window.ADMIN_LANG || \'' . addslashes($ui_strings['lang'] ?? 'en') . '\';</script>' . PHP_EOL;
    }
}
