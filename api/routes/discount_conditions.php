...
try {
    ...
    switch ($method) {
        case 'GET':
            ...
            $stmt = $pdo->prepare("SELECT * FROM discount_conditions WHERE discount_id = :discount_id ORDER BY id");
        ...
        case 'POST':
            ...
            $stmt = $pdo->prepare("INSERT INTO discount_conditions (discount_id, condition_type, operator, condition_value) VALUES (:discount_id, :condition_type, :operator, :condition_value)");
            $stmt->execute([
            ...
            ResponseFormatter::success(['id' => (int)$pdo->lastInsertId()], 'Condition created', 201);
            break;
        ...
        case 'PUT':
            ...
            $stmt = $pdo->prepare("UPDATE discount_conditions SET " . implode(', ', $sets) . " WHERE id = :id");
            $stmt->execute($params);
        ...
        case 'DELETE':
            $id = (int)($_GET['id'] ?? 0);
            if ($id <= 0) { ResponseFormatter::error('ID is required', 400); break; }
            $stmt = $pdo->prepare("DELETE FROM discount_conditions WHERE id = :id");
            $stmt->execute([':id' => $id]);
