...
final class PdoTenant_usersRepository
{
    ...
    public function all(int $tenantId, int $perPage = 10, int $offset = 0, array $filters = []): array
    {
        ...
        if (!empty($filters['search'])) {
            ...
            $whereParts[] = '(u.username LIKE :search_u OR u.email LIKE :search_e OR t.name LIKE :search_t)';
    ...
    public function save(int $tenantId, array $data, ?int $actingUserId = null): int
    {
        ...
        try {
            if ($isUpdate) {
                $sql = "
                    UPDATE tenant_users
                    SET role_id = :role_id, is_active = :is_active, updated_at = NOW()
                ...
                $sql = "
                    INSERT INTO tenant_users (tenant_id, user_id, role_id, joined_at, is_active)
                    VALUES (:tenantId, :user_id, :role_id, NOW(), :is_active)
                ";
                $stmt = $this->pdo->prepare($sql);
                $stmt->bindValue(':tenantId', $tenantId, PDO::PARAM_INT);
