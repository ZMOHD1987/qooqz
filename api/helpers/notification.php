...
class Notification {
    ...
    public static function send($userId, $type, $title, $message, $data = [], $channels = ['database']) {
        ...
        try {
            ...
            // جلب بيانات المستخدم
            $user = self::getUserData($mysqli, $userId);
            ...
            $settings = self::getUserNotificationSettings($mysqli, $userId, $type);
     ...
     * @return array
     */
    private static function saveToDatabase($mysqli, $userId, $type, $title, $message, $data = []) {
        $dataJson = ! empty($data) ? json_encode($data, JSON_UNESCAPED_UNICODE) : null;
        
        $sql = "INSERT INTO notifications (user_id, notification_type, title, message, data, is_read, created_at) 
                VALUES (?, ?, ?, ?, ?, 0, NOW())";
     ...
     * @return array|null
     */
    private static function getUserData($mysqli, $userId) {
     ...
     * @return array
     */
    private static function getUserNotificationSettings($mysqli, $userId, $type) {
        $sql = "SELECT email_enabled, sms_enabled, push_enabled 
