...
class Upload {
     ...
     * @param int $maxWidth الحد الأقصى للعرض (اختياري)
     ...
     */
    public static function uploadImage($file, $folder, $maxWidth = null, $maxHeight = null, $createThumbnail = false) {
        try {
     ...
     * @return array
     */
    public static function uploadDocument($file, $folder) {
        try {
            // التحقق من وجود الملف
            if (!isset($file) || !is_array($file)) {
                return self:: error('No file uploaded');
    ...
    // ===========================================
    // 3️⃣ رفع ملفات متعددة (Multiple Upload)
    // ===========================================
     ...
     * @return array
     */
    public static function uploadMultiple($files, $folder, $type = 'image') {
        $results = [];
        $successCount = 0;
        $failCount = 0;
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            if ($type === 'image') {
                ...
                $result = self:: uploadDocument($file, $folder);
            }
