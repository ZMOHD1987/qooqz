...
try {
    ...
    // 1) languages list (prefer languages table)
    $languages = [];
    ...
    // fallback: scan languages JSON folder if exists
    if (empty($languages)) {
    ...
    if (table_exists($conn, 'categories')) {
        // fetch columns to avoid selecting non-existent fields
        ...
        if (empty($selectCols)) {
            pm_log("categories table exists but has no expected columns");
        } else {
            ...
            // translations map for categories for requested language
    ...
    // 4) brands (with translations if present)
    $brands = [];
